...
if git -C "$cwd" rev-parse --git-dir >/dev/null 2>&1; then
    ...
    # Check for changes
   status_raw=$(git -C "$cwd" --no-optional-locks status --porcelain=v1 2>/dev/null)
       staged=$(echo "$status_raw" | grep -c "^[AMR]")
      deleted=$(echo "$status_raw" | grep -c "^ D")
     modified=$(echo "$status_raw" | grep -c "^ [AMR]")
    untracked=$(echo "$status_raw" | grep -c "^??")
      stashed=$(git -C "$cwd" --no-optional-locks stash list 2> /dev/null | wc -l)
        diffs=$(git -C "$cwd" --no-optional-locks rev-parse --verify origin/HEAD >/dev/null 2>&1 && git -C "$cwd" log --oneline origin/HEAD..HEAD | wc -l || echo 0)
